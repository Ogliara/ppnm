...
public static class mini{
	/*
	public static (vector,int) declipping(vector xr, vector xc, matrix Sr, matrix Sc, double acc=1e-3){
		func<vector,vector

		Func<vector,double> f = x =>{
			vector inner = matrix.product(Sr.transpose(),xr) + matrix.product(Sc.transpose(),x);
			vector grad1 = gradient();
		};
	}//declipping function (EXAM EXERCISE)
	*/
